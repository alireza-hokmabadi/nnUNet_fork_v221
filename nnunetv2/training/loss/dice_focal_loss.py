...
class DiceFocalBCELoss(nn.Module):
    def __init__(self, alpha=0.35, gamma=3, use_regions=False, use_ignore_label=False,
    ...
    def forward(self, inputs, targets):
        """
        inputs: logits (B, C, ...)
        targets: (B, 1, ...) or one-hot if regions
        """
        if self.use_regions:
            # already one-hot → shapes match
            ...
            # binary segmentation → take foreground channel
            inputs_fg = inputs[:, 1, ...]   # (B, ...)
            targets = targets.squeeze(1).float()  # (B, ...)

            dice = self.dice_loss(torch.sigmoid(inputs_fg), targets)
